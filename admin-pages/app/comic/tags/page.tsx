...
export default async function ComicTags({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  const { error } = await searchParams
  const tags = await listComicTags()

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">マンガタグ管理</h1>
      ...
      <form action={createComicTagAction} className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
