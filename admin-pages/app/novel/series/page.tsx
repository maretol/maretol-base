import { listNovelSeries } from '@/lib/db_novel'
import { createNovelSeriesAction } from '../actions'
import { SubmitButton } from '@/components/submit-button'

export const dynamic = 'force-dynamic'

export default async function NovelSeries({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  const { error } = await searchParams
  const series = await listNovelSeries()

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">小説シリーズ管理</h1>
      ...
      <form action={createNovelSeriesAction} className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
