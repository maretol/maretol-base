...
export function Pagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
      ...
      {totalPage > 1 && (
        <nav aria-label="ページネーション" className="flex flex-wrap items-center gap-1">
          {currentPage > 1 ? (
            <Link href={withPage(path, currentPage - 1)} className={linkClass}>
              前へ
            </Link>
          ) : (
          ...
          {pages.map((page) =>
            page === currentPage ? (
              ...
              <Link key={page} href={withPage(path, page)} className={linkClass}>
                {page}
              </Link>
            )
          )}
