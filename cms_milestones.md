...
- [ ] 本番切替（トップレベル ATELIER_SOURCE を 'd1' に変更済み。development → main のマージで本番デプロイされ完了）
...
- [ ] Cloudflare Access の設定（独自認証は作らない）
  - **Access設定完了までデプロイしない**（wrangler.toml は routes=[] / workers_dev=false で公開されない状態にしてある。管理用ドメインの決定もここで行う）
  - CIワークフロー（dryrun/stg/prd）への admin-pages ジョブ追加もデプロイ解禁時に行う
...
- [ ] ステージング・本番へのデプロイと実環境での動作確認（Cloudflare Access 設定後）
