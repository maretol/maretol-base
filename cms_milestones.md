...
- [ ] 本番切替（COMIC_SOURCE を 'd1' に変更済み。development → main のマージで本番デプロイされ完了）
...
- [ ] 【要リモート作業】本番データのインポート（`npm run export blog`（CMS_API_KEY）→ `npm run generate-sql:blog <contents> <categories> <info> <static>` → `wrangler d1 execute maretol-cms --remote --file=generated/blog_import.sql`）
...
- [x] SNS 通知の実装（admin-pages から Service Binding 経由で sns-article-publisher へ WebhookPayload 完全互換 + HMAC署名で送信。新規公開・下書き→公開のみ、is_secret除外。SNS_NOTIFY_ENABLED=true の環境のみ送信＝staging誤投稿防止。**要: admin-pages への SNS_PUB_CMS_KEY / SNS_PUB_CMS_SECRET シークレット設定**）
- [ ] 本番データのインポート（blog: export → generate-sql:blog → remote execute）
