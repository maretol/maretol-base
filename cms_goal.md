...
| maretol-blog     | static         | サイドバー等の静的文言 | sidebar*about, sidebar_profile, toppage*\*                                                                                                                                                   |
...
## 途中でmarkdownをhtmlに変換したテストの結果で気になったポイント

- 空行の変換を考える必要あり（ `</br>` を直接記述させていいか？）。ブログなので任意幅の空行は表現したい
- 表や引用の中のテキストを `<p>` に変換するか（しなくてもparsedContentに変換するのが支障がなければ問題ない
- 今後新規のコマンドやタグに対応したい場合どうするかを検討
