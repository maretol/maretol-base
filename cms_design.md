...
#### タイムスタンプを TEXT で持つ判断（M4 レビューで確認済み・変更しない）

- SQLite/D1 にネイティブ日時型はなく、TEXT（ISO8601）は公式にサポートされた保存規約のひとつ
- **不変条件: 常に UTC・ミリ秒付き・Z 終端の ISO8601**（例: `2026-07-04T13:53:58.642Z`）で保存する。この形式なら辞書順＝時系列順となり、ORDER BY・範囲比較が正しく動く
- 書き込み経路は「microCMS からのインポート（元の値をそのまま）」と「admin-pages の `new Date().toISOString()`」に限定されており、どちらもこの形式を満たす。新しい書き込み経路を作る場合も必ずこの形式に従うこと
- microCMS 互換 API が同形式の文字列を返す仕様のため、TEXT 保存なら入出力での変換が不要（epoch 整数にすると全境界で変換が必要になり、利点がない）

#### 内部ID / 外部IDを分離しない判断（M4 レビューで確認済み・変更しない）

- ID分離の主目的は「公開スラッグの事後変更」だが、本移行は「URL は移行前後で完全同一」が最終ゴールであり、公開IDは設計上そもそも不変。分離の利点を使う場面が原理的にない
- pages の URL・fetcher の APIパラメータ・KVキャッシュキー・draft キー・コンテンツ間参照（comic の next_id/previous_id 等）がすべて公開IDで統一されており、内部IDを挟むと全境界でマッピングの二重管理になる
- 将来必要になった場合も、この規模なら「INTEGER カラム追加 → バックフィル → FK 付け替え」で移行可能（今やらないことによる将来コストの増加はほぼない）
- トレードオフ: 公開後のスラッグ修正は「削除→再作成（＝URL変更）」になるが、これは URL 不変の要件と表裏一体であり受け入れる
